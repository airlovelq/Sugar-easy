...
class LeNet5(object):
    ...
    def train(self, dataset_path, **train_args):
        ...
        pil_images, image_classes, num_samples, num_classes = load_image_classification_dataset(dataset_path, 'L')
    ...
    def predict(self, query):
        ...
        pil_image = Image.open(encoded).convert('L')
        ...
        probs = self._model.predict_proba(images)
    ...
    def save(self, path):
        # model_data_path = os.path.join(path, 'model_data')
        # with open(h5path, 'a') as f:
        #     pass
    ...
    def load(self, path):
        # h5path = os.path.join(path, 'model.h5')
    ...
    def _trans_image(self, pil_images):
        ...
        images = np.asarray(images) / 255
        X = np.asarray(images)
        return X.reshape(-1, self._image_size, self._image_size, 1)
    ...
    def _build_classifier(self, l_rate):
        ...
        model.add(
            layers.Conv2D(filters=6,
                          kernel_size=(3, 3),
                          activation='relu',
                          input_shape=(self._image_size, self._image_size, 1)))
        ...
        model.add(layers.Dense(units=self._num_classes, activation='softmax'))
        adam = Adam(lr=l_rate,
                    beta_1=0.9,
                    beta_2=0.999,
                    epsilon=None,
